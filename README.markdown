...
In so doing, the following happened:

  * CommandT became very quick. Previously on my system, there was a notable
    delay.
  * No more delays on keyrepeats.
...
An alternative buffer swapping tool. Allows you to quickly move between last
used buffers quickly. Currently mapped to <Leader>lj. After hitting this
shortcut, you can then press 'ss' to "Alt-Tab" to the last used buffer, making
it easier to swap between two active buffers. If you come from TextMate, this is
similar to using CMD-T to "Alt-Tab" between last open file.

Since VIM buffers are far more powerful, this plugin will get you back to the
usual habbits of an "Alt-Tab" like behaviour.
